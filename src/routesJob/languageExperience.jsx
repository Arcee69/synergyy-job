...
import { worldLanguages, level } from '../utils/data';
    ...
    return (
        <Onboards full={true} rate={'w-[44.44%]'} width={true}>
            <div className='flex mb-12 items-center justify-between px-8'>
                <h1 className='capitalize text-[18px] font-medium text-white'>Add Languages</h1>
                <Exit onClick={() => navigate(-1)}/>
            </div>
            <div className='px-8 -mt-5 flex flex-col gap-8'>
                ...
                <div className='flex flex-col w-full gap-[26.35px] mb-10'>
                    <div className='w-full flex justify-center'>
                        ...
                        :
                            <Buttons text={'Save'} padding={'12px 48px'} functions={submitForm} width={'318px'} hoverColor={inputData.length > 0 ? '#FBA599' : '#BABABA'} bgColor={inputData.length > 0 ? '#FBA599' : '#BABABA'} color={'#00141B'}/>
                        }
                    </div>
                </div>
            </div>
