    ...
    return (
        <div className='flex-1 phone:mt-8 flex flex-col phone:gap-16 gap-[150px]'>
            <div className='flex flex-col phone:w-[421px] w-full'>
                ...
                {/* get */}
                {selected?.length > 0 && (
                    <div className='phone:h-[251px] overflow-y-scroll mt-[57.6px] flex flex-col gap-[14.4px]'>
                        {selected?.length > 1 && <p className='text-[13.2px] text-[#BBD4DD]'>{selected?.length} skills added</p>}
                        {selected?.map((s) => {
                            return (
                                <div className='border-[1.44px] border-[#10303D] cursor-pointer items-center flex justify-between p-3 w-full rounded-lg bg-backgroundDark'>
            ...
            <div className='phone:mt-0 -mt-[150px] pb-12 flex-1 items-end phone:flex hidden w-full justify-center'>
                {isLoading ? (
                    ...
                    <Buttons functions={selectCoreSkill} text={'Continue'} hoverColor={selected?.length > 0 ? '#FBA599' : '#BABABA'} width={'318px'} color='#00141B' bgColor={selected?.length > 0 ? '#FBA599' : '#BABABA'} />
                )}
            </div>
            <div className='phone:hidden pb-12 items-end flex flex-1 w-full justify-center'>
                {isLoading ? (
                    <Buttons text={''} icons={<CircularProgress size={28} sx={{ color: 'white' }}/>} hoverColor={'#FBA599'} width={'318px'} color='#00141B' bgColor={'#FBA599'} />
                ) : (
                    <Buttons functions={selectCoreSkill} text={'Next'} hoverColor={selected?.length > 0 ? '#FBA599' : '#BABABA'} width={'318px'} color='#00141B' bgColor={selected?.length > 0 ? '#FBA599' : '#BABABA'}/>
                )}
            </div>
        </div>
    )
