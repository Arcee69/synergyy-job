...
export default function EducationHistoryForm2() {
    const [tick, setTick] = useState(false);
    const initialValues = {school:'', degree:''}
    const [educationHistoryData, setEducationHistoryData] = useState(initialValues)
    ...
    const handleSubmit = async() => {
        setIsLoading(true)
        if (!educationHistoryData.school || !educationHistoryData.degree || !monthStarted || !yearStarted){
            setIsLoading(false)
            return
        }
        const data = {
            school: educationHistoryData.school,
            degree: educationHistoryData.degree,
            start_date: yearStarted + '-' + monthStarted.number,
            ...!tick && {end_date: yearEnded + '-' + monthEnded.number}
        }
        
        try {
            const response = await axios.post(`${baseURL}account/add_education`, data, {
                headers: {Authorization: `Bearer ${accessToken}`,}
            });
            if (response.data.status ==='success'){
                setIsLoading(false)
                setEducationHistoryData(initialValues)
    ...
    return (
        <div className='phone:mt-8 -mt-1 w-full flex flex-col phone:gap-[38.4px] gap-8'>
            <div className='flex flex-col phone:gap-[9.6px] gap-2'>
                <label className='phone:text-[18px] text-[15px] text-[#99A6AB]'>School name</label>
                <input
                    type="text"
                    placeholder="University/School/Authority"
                    id="name"
                    name="school"
                    ...
                    value={educationHistoryData.school}
                    onChange={(e) => {
                        setEducationHistoryData(val => {
            ...
            <div className='flex flex-col phone:gap-[9.6px] gap-2'>
                <label className='phone:text-[18px] text-[15px] text-[#99A6AB]'>Degree or Certification</label>
                <input
                    type="text"
                    placeholder="E.g Bsc Accounting, Design, etc..."
                    id="name"
                    name="degree"
                    className="caret-[#42B8BD] w-full border-[#334D57] bg-backgroundDark border-[1.2px] rounded-[4.8px] sm:py-[14.4px] py-3 h-10 sm:h-12 sm:px-[19.2px] px-4 pr-4 focus:outline-none text-[16px] text-[#CCD3D5] placeholder:text-[#667A81]"
                    value={educationHistoryData.degree}
                    onChange={(e) => {
                        setEducationHistoryData(val => {
                            return {...val, [e.target.name]:e.target.value}
                        })
            ...
            <div className='flex items-center phone:gap-[9.6px] gap-2'>
                ...
                <p onClick={() => setTick(!tick)} className='cursor-pointer text-white phone:text-16.8px'>Currently enrolled</p>
            </div>
            {!tick && (
                <div className='flex phone:gap-[19.2px] gap-4 w-full'>
                    <div className='w-full flex flex-col phone:gap-[9.6px] gap-2'>
            ...
            <div className='mt-32 mb-6  w-full justify-center flex flex-1 items-end'>
                {isLoading ? (
                    ...
                    <Buttons functions={handleSubmit} text={'Save'} hoverColor={(educationHistoryData.school && educationHistoryData.degree && monthStarted && yearStarted) ? '#FBA599' : '#BABABA'} width={'318px'} color='#00141B' bgColor={(educationHistoryData.school && educationHistoryData.degree && monthStarted && yearStarted) ? '#FBA599' : '#BABABA'}/>
                )}
            </div>
        </div>
    )
